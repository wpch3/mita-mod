...
- **真结局「现实一起生活」**：全员存活 + 集齐两件伏笔道具（核心数据盘+修复补丁），米塔们的数据随主角进入现实，在现实的房间里一起生活。
...
| **真结局「现实一起生活」** | `AllRescued && HasCoreDataDisk && HasRepairPatch` | 复用 `Scene 1 - RealRoom`（现实房间）：主角把数据盘接入现实终端，米塔们的数据实体化走进现实房间，米拉出现在**现实电视屏幕**上——"以后就是室友了"式收尾 |
| 原版结局 | 上述均不满足 | 按原版运行，**零痕迹** |

- 同一周目两结局都满足时：在终章终点（候选 `Core_Entry`）做**分歧选择点**（两扇门/两个终端选项）——
  向左：留在这个世界与她们告别（完美）；向右：带她们回现实（真）。实现细节阶段 2 定。
