...
0. **追杀章出现"消失→回入口→轮回对话"的循环**（2026-08-26 实证，Location6 电锯章）：
   ```ini
   PacifyMitas = false      # 追杀剧本必须有个"收场动作"才能落幕，全拦了会超时重置无限轮回
   GodMode = true           # 保住：她碰到你也杀不死你
   SpeedMultiplier = 2.5    # 提高：让她根本追不上你，收场判定自然不触发，走到出口剧情正常推进
   ```
   打法上**别站桩躲藏**，沿着逃跑路线正常跑——2.5 倍速下她连你影子都摸不到，
   整个追杀剧本会"干净地空转"到出口 `PlayerExit` 自然结束。
   原理：拦截"收场动作"不等于杀死剧本，计时器到点就重置 = 无限补考。
   速度（让她追不上）才是追杀章的正解，拦截（让她打不死）只是保底。
